...
<script lang="ts">
  ...
  function createQueryFromDoc(doc: ProseMirrorNode): string {
    ...
    doc.descendants((node) => {
      if (node.isText) {
        console.log('[createQueryFromDoc] Text node:', node.text, 'marks:', node.marks.map(m => m.type.name));
        query += node.text;
        return false;
      }
      ...
      if (node.type.name === "term") {
        ...
        console.log('[createQueryFromDoc] Term node:', { field, value, boost, prefix, quoted });
    ...
    console.log('[createQueryFromDoc] Final query:', query);
    return query;
  }
    ...
    state: {
      ...
      apply(tr, value) {
        ...
        if (tr.getMeta("rebuild")) {
          console.log('[QueryTracking] Skipping rebuild transaction');
          return value;
        }
        // Skip recomputation if this is an autocomplete intermediate step
        // (e.g., inserting a field term with empty value before value selection)
        if (tr.getMeta("autocomplete-intermediate")) {
          console.log('[QueryTracking] Skipping autocomplete-intermediate transaction');
          console.log('[QueryTracking] Doc structure:', JSON.stringify(tr.doc.toJSON(), null, 2));
          return value;
        }
        ...
        if (tr.docChanged) {
          ...
          console.log('[QueryTracking] Doc changed, new query:', newQuery);
          console.log('[QueryTracking] Doc structure:', JSON.stringify(tr.doc.toJSON(), null, 2));
  ...
  onMount(() => {
    ...
    const state = EditorState.create({
      ...
      plugins: [
        ...
        autocompletePlugin, // Must be before baseKeymap to intercept Enter
        keymap(baseKeymap),
        queryTrackingPlugin,
      ],
    });
