...
- **`viewer-deep-link.spec.ts`** — regression coverage for #1203: opening `…/#document/pN` must actually come to rest on page N, not several pages short of it. Measures the error in _pages_ (`pagesOffTarget`) so an assertion means the same thing at any viewport or zoom. Two of the four cases are deterministic guards for the causes rather than the symptom: that `.pages` spacing is never re-laid out after the first paint, and that the viewer re-aligns when the layout shifts _after_ it scrolls. Beware that Chrome keeps re-scrolling to the URL fragment while the document is still loading, which masks the symptom on wide screens — the late-shift case hangs its injected shift off the viewer's own `scrollIntoView` so it lands after that window.
