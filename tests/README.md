...
- **`viewer-deep-link.spec.ts`** — regression coverage for #1203: opening `…/#document/pN` must actually come to rest on page N, not several pages short of it. Measures the error in _pages_ (`pagesOffTarget`) so an assertion means the same thing in any mode or viewport. Two cases are deterministic guards for the causes rather than the symptom: that `.pages` spacing is never re-laid out after the first paint, and that the viewer re-aligns when the layout shifts _after_ it scrolls. Two more cover text mode, whose pages render behind a promise (so the target doesn't exist when the deep link fires) — including that a client-side mode switch holds your place, which keeping the raw pixel offset does not do, since text pages aren't PDF-page-height. Three cover numeric zoom, where a page's placeholder box and its rendered box used to disagree; the guard there asserts the two are the same size, which holds only because every page in the multi-page fixture is 595x842. Three gotchas: Chrome keeps re-scrolling to the URL fragment while the document loads, which masks the symptom on wide screens, so the late-shift case hangs its injected shift off the viewer's own `scrollIntoView`; the mode tabs' accessible name isn't their visible label (the octicon contributes to it) and the collapsed dropdown holds a hidden copy of each, so they're located by `href` plus `:visible`; and a logged-in reading toolbar collapses that switcher below ~1440px, which is why the text-mode cases run wider than the rest.
